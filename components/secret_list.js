...
class SecretList extends React.Component {
    ...
    async load() {
        let json = await myexec('kubectl get secrets');
                    ...
                    </tr>
                    {
                        this.state.secrets && this.state.secrets.map((secret, index) => {
                            return (
                                <tr key={index}>
                                    <td>
                                        <a href="javascript:void(0)" onClick={() => describe('secret', secret.name)}>{secret.name}</a>
                                    </td>
                                    <td>{secret.type}</td>
                                    <td>{secret.data}</td>
                                    <td>{secret.age}</td>
                                    <td>
                                        <a href="javascript:void(0)" onClick={() => showYamlEditor('secret', secret.name)}>YAML</a>
                                    </td>
                                </tr>
                            )
                        })
